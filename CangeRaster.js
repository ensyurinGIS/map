...
slider_raster_saturation.addEventListener('input', (e) => {
    const sliderValue = document.getElementById(
        'slider_raster_saturation_value',
    );
    map.setPaintProperty(
        property_raster.value,
        'raster-saturation',
...
slider_raster_brightnessmin.addEventListener('input', (e) => {
    const sliderValue = document.getElementById(
        'slider_raster_brightnessmin_value',
    );
    map.setPaintProperty(
        property_raster.value,
...
slider_raster_brightnessmax.addEventListener('input', (e) => {
    const sliderValue = document.getElementById(
        'slider_raster_brightnessmax_value',
    );
    map.setPaintProperty(
        property_raster.value,
...
function CangeRaster() {
    ...
    document.getElementById('slider_raster_opacity').value =
        Math.trunc(nowopacity);
    sliderValueO.textContent = Math.trunc(nowopacity) + '%';

    var nowsikiso =
        map.getPaintProperty(layer.value, 'raster-saturation') * 100;
    const sliderValueS = document.getElementById(
        'slider_raster_saturation_value',
    );
    document.getElementById('slider_raster_saturation').value =
        Math.trunc(nowsikiso);
    sliderValueS.textContent = Math.trunc(nowsikiso);

    var nowcontrast =
        map.getPaintProperty(layer.value, 'raster-contrast') * 100;
    const sliderValueC = document.getElementById(
        'slider_raster_contrast_value',
    );
    document.getElementById('slider_raster_contrast').value =
        Math.trunc(nowcontrast);
    sliderValueC.textContent = Math.trunc(nowcontrast);

    var nowbmin =
        map.getPaintProperty(layer.value, 'raster-brightness-min') * 100;
    const sliderValueMIN = document.getElementById(
        'slider_raster_brightnessmin_value',
    );
    document.getElementById('slider_raster_brightnessmin').value =
        Math.trunc(nowbmin);
    sliderValueMIN.textContent = Math.trunc(nowbmin);

    var nowbmix =
        map.getPaintProperty(layer.value, 'raster-brightness-max') * 100;
    const sliderValueMAX = document.getElementById(
        'slider_raster_brightnessmax_value',
    );
    document.getElementById('slider_raster_brightnessmax').value =
        Math.trunc(nowbmix);
